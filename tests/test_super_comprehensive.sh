...
run_minishell() {
    local command="$1"
    # Crear un script temporal para capturar solo la salida del comando
    local temp_script="${TMP_DIR}/run_command.sh"
    echo "#!/bin/bash" > "$temp_script"
    echo "echo '$command' | $MINISHELL 2>&1" >> "$temp_script"
    chmod +x "$temp_script"
    
    # Ejecutar y filtrar la salida para extraer solo la respuesta al comando
    timeout "$TIMEOUT_DURATION" "$temp_script" > "${TMP_DIR}/raw_minishell_output.txt" 2>&1
    local exit_code=$?
    
    # Procesar la salida para extraer solo la respuesta del comando
    # Buscar la línea que contiene el comando y capturar todo hasta el siguiente prompt
    awk '
    BEGIN { capturing = 0; found_command = 0 }
    /alejandro@Minishell.*: / {
        if (found_command && capturing) {
            exit  # Salir cuando encontremos el siguiente prompt
        }
        if (!found_command) {
            # Verificar si esta línea contiene nuestro comando
            if (index($0, "'"$command"'") > 0) {
                found_command = 1
                capturing = 1
                next  # No imprimir la línea del prompt con el comando
            }
        }
        next
    }
    ...
    ' "${TMP_DIR}/raw_minishell_output.txt" > "$MINISHELL_OUTPUT"
...
compare_outputs() {
    # Limpiar caracteres ANSI y líneas vacías
    sed 's/\x1b\[[0-9;]*m//g' "$MINISHELL_OUTPUT" | sed '/^$/d' > "${TMP_DIR}/minishell_clean.txt"
    sed '/^$/d' "$BASH_OUTPUT" > "${TMP_DIR}/bash_clean.txt"
    
    # Si ambos archivos están vacíos, considerar igual
    if [[ ! -s "${TMP_DIR}/minishell_clean.txt" && ! -s "${TMP_DIR}/bash_clean.txt" ]]; then
        return 0
    fi
    
    # Comparar contenido relevante
    if diff -u "${TMP_DIR}/minishell_clean.txt" "${TMP_DIR}/bash_clean.txt" > "${TMP_DIR}/diff_output.txt" 2>&1; then
        return 0
    else
        return 1
    fi
